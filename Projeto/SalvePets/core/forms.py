...
from .models import ADOCAO, INSTITUICAO, Pet, USUARIO
...
class UsuarioForm(forms.ModelForm):
    ...
    """receberNotificacoes = forms.BooleanField(required=False,
                                                ...
                                                help_text='(Marque este campo caso deseje ser notificado sobre pets perdidos ou encontrados.)')"""
    class Meta:
        model = USUARIO
        fields = ('cpfCnpj', 'dataNascimento', 'telefone',
                  'site', 'receberNotificacoes') #Retirei tipoUsuairo


class ExtendedSignupForm(SignupForm):
    """ Classe que seria usada como "Cadastro Único """
    # li que é possível fazer funcionar, só preciso entender melhor como
    ...
    receberNotificacoes = forms.BooleanField(required=False,
                                             label = _('Desejo receber notificações'),
                                             help_text=_('(Marque este campo caso deseje ser notificado sobre pets perdidos ou encontrados.)'))
    #siteUrl = forms.URLField(initial='http://', label='URL do seu Site')

#Projeto integrado II
class ContactForm(forms.Form):
    """classe para definir as medidas de um formulário para adicionar uma instituição"""
...
class InstituicaoForm(forms.ModelForm):
    """classe para criar um formulário para adicionar uma instituição"""
    class Meta:
        model = INSTITUICAO
        fields = ('nome_instituicao', 'razao_social', 'cnpj', 'telefone', 'email')

class AdicionarUsuarioInstituicaoForm(forms.Form):
    """classe para criar um formulário para adicionar usuário ligado a uma instituição"""
